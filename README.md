...
standings, full fixtures, per-match detail (goals, assists, cards, subs, team
stats, lineups), team profiles, and tournament-wide statistics — built end-to-end
on a **$0 hosting budget**.
...
| Route | What it does |
| --- | --- |
| `/` | **Home** — today's matches with live scores (auto-refreshing), plus a tournament-stats teaser (top scorers, most goals, best defense, goals played). |
| `/match/[id]` | **Match detail** — scoreline, full event timeline (goals **with assists**, yellow/red cards, substitutions), team stats (possession, shots, shots on target, passing, etc.), and lineups with sub-on/off indicators. |
| `/standings` | **Standings** — all 12 group tables. |
| `/fixtures` | **Fixtures** — the full tournament schedule. |
| `/teams` & `/teams/[id]` | **Teams** — every nation, each with World Cup history (titles, appearances, best finish), FIFA ranking, squad age profile (average age, youngest/oldest, U-23 count), the full squad by position, recent form, and next match. |
| `/stats` | **Stats** — tournament leaderboards: top scorers, top assisters, team goals (most scored / fewest & most conceded), best passing team, and discipline (cards by player and team). |

Live data updates within ~30s without a page reload. The whole thing runs at the
edge with **no database**.
...
```
src/
├─ app/                 # routes (home, match, standings, fixtures, teams, stats, api)
├─ components/          # UI (MatchCard, MatchDetailView, StatsView, TeamProfileView, Nav, …)
├─ hooks/               # TanStack Query hooks (useScoreboard, useMatchDetail, useStats)
├─ lib/
